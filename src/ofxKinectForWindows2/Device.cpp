...
namespace ofxKinectForWindows2 {
	...
	//----------
	void Device::drawWorld(DrawStyle drawStyle) {
		...
		if (depthSource) {
			...
			if (drawStyle & (DrawStyle::Pretty | DrawStyle::Vertices)) {
				//draw point cloud
				mesh.drawVertices();
			}

			if (drawStyle & (DrawStyle::Pretty | DrawStyle::Wireframe)) {
				//draw triangles
				ofSetColor(255, 150);
				mesh.drawWireframe();
			}

			if (drawStyle & (DrawStyle::Pretty | DrawStyle::Faces)) {
				//draw fills faded
				ofSetColor(255, 50);
				mesh.drawFaces();
			}
